...
import Schema from "!!raw-loader!@site/../demos/users-and-auth/convex/schema.ts";
...
[Schema Philosophy](/database/advanced/schema-philosophy.mdx).
...
[System Fields](/database/types.mdx#system-fields).

<Admonition type="tip" title="Generating a Schema">

While writing your schema, it can be helpful to consult the
[Convex Dashboard](/dashboard/deployments/data.mdx#generating-a-schema). The
...
When you change a field type or reshape documents, existing data may not match
your new schema. Use `.migrate()` on validators or tables to declare inline data
migrations that run automatically during `convex dev` and `convex deploy`, before
schema validation.

Field-level migrations transform a single field:
...
export default defineSchema({
  numbers: defineTable({
    ...
    value: v.string().migrate("number to string", (ctx) => {
      if (typeof ctx.oldValue === "number") {
        return `${ctx.doc.key}: ${ctx.oldValue.toString()}`;
      }
    }),
  }),
});
...
Table-level migrations can patch multiple fields at once:
...
**Migration handler semantics:**

- Return a value (field) or partial patch object (table) to update the document.
- Return `undefined` to skip a document (already migrated or not applicable).
- Throw to fail the push and surface the error.

Each migration has a stable string `id`. Once a migration runs in an environment,
Convex records that id and skips re-running it. If you change the handler code
under the same id, the push fails — use a new id instead.

In development, `convex dev` prompts before running pending migrations. In
production, migrations run automatically. For complex multi-table or long-running
migrations, use [`@convex-dev/migrations`](/production/migrations.mdx).

## Schema validation

Schemas are pushed automatically in [`npx convex dev`](/cli/reference/dev.mdx)
and [`npx convex deploy`](/cli/reference/deploy.mdx).
...
<Snippet title="convex/users.ts" source={circularExample} />

[Let us know](/production/contact.mdx) if you need better support for circular
references.

## TypeScript types

Once you've defined a schema, [`npx convex dev`](/cli/reference/dev.mdx) will
produce new versions of [`dataModel.d.ts`](/generated-api/data-model) and
